...
async function lomuto(ele, l, h) {
    ...
    for (let j = l; j <= h - 1; j++) {
        ...
        if (parseInt(ele[j].style.height) < parseInt(ele[h].style.height)) {
            ...
            // Update displayed values after swap
            const temp = ele[i].querySelector('.bar-value').textContent;
            ele[i].querySelector('.bar-value').textContent = ele[j].querySelector('.bar-value').textContent;
            ele[j].querySelector('.bar-value').textContent = temp;
    ...
    // Update displayed values after swap
    const temp = ele[i + 1].querySelector('.bar-value').textContent;
    ele[i + 1].querySelector('.bar-value').textContent = ele[h].querySelector('.bar-value').textContent;
    ele[h].querySelector('.bar-value').textContent = temp;
...
document.getElementById('quick').addEventListener('click', async function () {
    ...
    // Update displayed values after sorting
